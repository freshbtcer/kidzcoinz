...
#if defined(NDEBUG)
# error "KidzCoinz cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe7ccec73c48d24d8abee7b43ca0bfc05229bdb394ea8fa48da40a7f5e90eac3f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KidzCoinz: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KidzCoinz Signed Message:\n";
...
{
    // KidzCoinz: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 2628000); // KidzCoinz: 2.628mil blocks in ~5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // KidzCoinz: 1 days
static const int64 nTargetSpacing = 1 * 60; // KidzCoinz: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KidzCoinz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // KidzCoinz: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe7ccec73c48d24d8abee7b43ca0bfc05229bdb394ea8fa48da40a7f5e90eac3f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "KidzCoinz September 30, 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04925f5e757a76fbb76f7c5a4dec4c7cf8291c7a93a956ffcc4b9a7e116b3805ae4dce543aecf8cd01cf3558c87ef6622394dbe76c3b7b8dee50de57db8afb5d30") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1506781346;
        ...
        assert(block.hashMerkleRoot == uint256("0x1aab8dc4b23fa4b57afcfec710da29469b13ba8deeeb54121aca27d4ae392e03"));
...
unsigned char pchMessageStart[4] = { 0xff, 0xc3, 0xb1, 0xdd }; // KidzCoinz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KidzCoinzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KidzCoinzMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KidzCoinzMiner(CWallet *pwallet)
{
    printf("KidzCoinzMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kidzcoinz-miner");
    ...
    try { loop {
        ...
        printf("Running KidzCoinzMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KidzCoinzMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KidzCoinzMiner, pwallet));
}
